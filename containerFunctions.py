...
def getFeaturesPoints(preprocessedVid,maskpath,dscale,boxSize,TemporalLength,numbofSamples,patchSize,numFramesAvg):
    ...
    isWater = np.zeros((1,numbofSamples),dtype=bool)# 1 *3 500
    temporalArr = np.zeros((numbofSamples, TemporalLength))# 3500 * 200
    spatialArr = np.zeros((numbofSamples, 256)) # 3500 * 256
    minrand = max(int(boxSize/2+1),int(patchSize/2))# 5
    totalFeatures= np.zeros((numbofSamples,TemporalLength+256))# 3500 * 456

    #gets array with amount of features
    for i in range(numbofSamples):
        randx = random.randrange(minrand, width - minrand) # generate randoom number within given range. but (width-minrand) is not exclusive
...
def LoopsThroughAllVids(pathToVidsFolder,pathToMasksPondFolder,pathToOtherTextures,numFrames, dFactor, densityMode,boxSize,TemporalLength,numbofSamples,patchSize,numFramesAvg):
    ...
    #goes through first 20 vids ; in here they train the model for whole videos
